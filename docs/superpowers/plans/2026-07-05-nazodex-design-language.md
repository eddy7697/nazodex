# NazoDex 設計語言 UI 導入 Implementation Plan
...
- 原始 PNG 母檔目錄 `public/nazodex_assets/` 不進 repo(gitignore),處理後輸出檔才 commit。
- TDD:純函式與元件先寫失敗測試。
...
- Modify: `.gitignore`(加 `public/nazodex_assets/`)
...
```bash
rm scripts/convert-assets.mjs public/nazodex_assets/*.webp
```
